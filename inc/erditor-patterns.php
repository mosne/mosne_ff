			...
			foreach ( $files as $file ) {
				...
				if ( empty( $pattern_data['slug'] ) ) {
					_doing_it_wrong(
						...
						/* translators: %s: file name. */
							esc_html__( 'Could not register file "%s" as a block pattern ("Slug" field missing)', 'mosne' ),
							esc_html( $file )
						),
						'6.0.0'
					);
				...
				if ( ! preg_match( '/^[A-z0-9\/_-]+$/', $pattern_data['slug'] ) ) {
					_doing_it_wrong(
						...
						/* translators: %1s: file name; %2s: slug value found. */
							esc_html__( 'Could not register file "%1$s" as a block pattern (invalid slug "%2$s")', 'mosne' ),
							esc_html( $file ),
				...
				if ( ! $pattern_data['title'] ) {
					_doing_it_wrong(
						...
						sprintf(
						/* translators: %1s: file name; %2s: slug value found. */
							esc_html__( 'Could not register file "%s" as a block pattern ("Title" field missing)', 'mosne' ),
							esc_html( $file )
						),
						'6.0.0'
					);
...
add_action( 'init', 'register_patterns', 11 );
