...
function m_edit() {
    if (current_user_can('level_3')) { echo edit_post_link('edit', '', '');};
}
...
function m_check($input, $tag=null, $class=null, $strip="false"){
    if($input){
        if($strip=='true'){ $input_s = strip_tags($input);}
        elseif($strip!=='false'){$input_s = strip_tags($input, $strip); }
        else{ $input_s=$input;}
        if($tag){
            if($class){ $otag = '<'.$tag.' class="'.$class.'">';}
            else{ $otag='<'.$tag.'>';}
