...
function m_get_dimensions( $svg ) {
    ...
    $width = (integer) $viewbox_v[2];
    $height = (integer) $viewbox_v[3];
...
function mosne_svg($svg, $mid) {
    ...
    if ( false === ( $svg_md5_cache = get_transient( $trans_p.$mid ) ) ) {
        ...
        if (!$svgx["preserveAspectRatio"]) {$svgx->addAttribute('preserveAspectRatio', 'xMinYMin meet');}
        if (!$svgx["class"]) {$svgx->addAttribute('class', 'svg-content');}
        $document = $svgx->asXML();
    ...
    if ($image["mime_type"]=="image/svg+xml"){
        ...
        if ($image) {
            ...
            if ($image["mime_type"]=='image/gif'){
                echo  '<div '.m_wrapper($image,$sz).'><img src="'.m_placehoder().'" alt="'.$image["title"].'" '.$extraok.'/></div>';
            }else{

                if($video_url!=""){ echo '<div class="video-push"><a href="'.$video_url.'" class="embedvideo">';}
                echo '<div '.m_wrapper($image,$sz).'><img class="lazyload" src="'.m_placehoder().'" data-sizes="auto" data-srcset="'.$srcset.'" alt="'.$image["title"].'"/></div>';
                if($video_url!=""){ echo '</a></div>';}
            }
        }
    }
}
